...
function normLabel(s){return String(s||"").toLowerCase().normalize("NFD").replace(/[`-?]/g,"").replace(/\b1ere\b|\b1re\b/g,"1e").replace(/\b2nde\b|\b2nd\b/g,"2nde").replace(/\b3eme\b/g,"3e").replace(/\b4eme\b/g,"4e").replace(/\b5eme\b/g,"5e").replace(/\b6eme\b/g,"6e").replace(/\bterminale\b/g,"tle").replace(/\s+/g," ").trim();}
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 18px 18px"}}>
        ...
        :vieSco.map((v,i)=>(
          <div key={v.id||i} style={{display:"flex",gap:10,padding:"8px 0",borderBottom:"1px solid #f1f5f9",alignItems:"flex-start"}}>
            <span style={{fontSize:14,flexShrink:0}}>{v.type==="retard"?"?":v.type==="sanction"?"??":"??"}</span>
              ...
              return (
                <div key={e.id} style={{display:"flex", alignItems:"center", gap:10, padding: isMobile?"10px 12px":"9px 14px",
                  ...
                  <span style={{width:16, flexShrink:0, fontSize:13}}>
                    {status==="pending"?"?":status==="saved"?"?":status==="error"?"??":""}
                  </span>
                </div>
              );
            })}
      ...
      <div style={{background:C.white, borderBottom:`1px solid ${C.border}`, padding:"0 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:0, flexShrink:0}}>
        <div style={{display:"flex"}}>
          {[
            ...
            {id:"gerer", label:"?? Gérer",            sub:"Ajouter · Supprimer"},
    ...
    <div style={{display:"flex", flexDirection:"column", gap:12}}>
      ...
      {conflits.length > 0 && (
        <div style={{background:C.redPale, border:`1px solid ${C.redBorder}`, borderRadius:12, padding:"16px 18px", display:"flex", flexDirection:"column", gap:14}}>
          <div>
            <h3 style={{fontSize:13, fontWeight:800, color:C.red, margin:0}}>?? {conflits.length} classe{conflits.length>1?"s":""} assignée{conflits.length>1?"s":""} à plusieurs enseignants</h3>
  ...
  return (
    <div style={{padding:"20px", display:"flex", flexDirection:"column", gap:16}}>
      ...
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}>
        <div>
          <h2 style={{fontSize:16, fontWeight:800, color:C.txt, margin:0}}>?? Gestion des enseignants</h2>
    ...
    <div style={{padding:"20px", display:"flex", flexDirection:"column", gap:16}}>
      ...
      <div style={{display:"grid", gridTemplateColumns: isMobile?"repeat(2,1fr)":"repeat(4,1fr)", gap:10}}>
        {[
          ...
          {label:"En alerte <50%",   val:alertes, emoji:"??", col:alertes>0?C.red:C.green, bg:alertes>0?C.redPale:C.greenPale},
        ].map((k,i)=>(
      ...
      {selClasse && (
        <>
          ...
          <div style={{ display:"flex",gap:10,flexWrap:"wrap",alignItems:"center" }}>
            ...
            {saving && <Pill ch="?? Sauvegarde…" color={C.blue}/>}
            {!isAdmin && <Pill ch="?? Cliquez sur une leçon pour la cocher/décocher" color={C.txtMuted}/>}
          </div>
      ...
      {isAdmin && ep.statut !== "attente" && (
        <div style={{ padding:"10px 16px",borderBottom:`1px solid ${C.border}` }}>
          {ep.stockPath ? (
            ...
            <span style={{ fontSize:12,fontWeight:700,color:C.red }}>?? Fichier manquant — non envoyé lors de la soumission</span>
          )}
        </div>
      )}
...
function EpViewAdmin({ data, setData, showToast }) {
  ...
  const handleAction = useCallback(async (epId, statut, commentaire) => {
    ...
    if (statut) {
      ...
      if (ok) {
        showToast(statut==="validee" ? "? Épreuve validée — enseignant notifié" : "?? Épreuve rejetée — enseignant notifié", true);
      }
    }
    if (commentaire) {
...
function EpViewTeacher({ user, data, setData, showToast }) {
  ...
  const onSubmit = (ep, ok, uploadFailed) => {
    if (ok && ep) {
      ...
      if (uploadFailed) {
        showToast("?? Fiche enregistrée mais le FICHIER n'a pas pu être envoyé — réessayez ou contactez l'animatrice", false);
      } else {
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
            ...
            {rejected.length > 0 && (
              <div style={{ background:C.redPale,border:`1px solid ${C.redBorder}`,borderRadius:10,padding:"10px 14px",marginBottom:4 }}>
                <div style={{ fontSize:12,fontWeight:700,color:C.red }}>?? {rejected.length} épreuve{rejected.length>1?"s":""} rejetée{rejected.length>1?"s":""} — action requise</div>
...
<table class="main-tbl">
  ...
  <thead>
    <tr>
      ...
      <th class="th-l1" colspan="3">Taux de réussite <sup>(4)</sup> des élèves (M = 10)</th>
      <th class="th-l1" rowspan="3">Moyenne Générale<br>de la classe <sup>(5)</sup><br>/20</th>
    </tr>
    <tr>
        ...
        <div style={{ display:"flex", flexDirection: isMobile?"column":"row", gap:8, alignItems: isMobile?"stretch":"center" }}>
          ...
          {previewHtml && (
            <button onClick={imprimerApercu}
              ...
              ??? {isMobile?"Imprimer":"Imprimer ce document"}
            </button>
          )}
        </div>
      </div>
          ...
          <div style={{ padding: isMobile?"10px 14px":"10px 20px", borderBottom:`1px solid ${C.border}`, display:"flex", flexWrap:"wrap", alignItems:"center", gap:10, flexShrink:0, background:C.white }}>
            <div style={{ flex:1, minWidth:"60%" }}>
              ...
              <p style={{ margin:"2px 0 0", fontSize: isMobile?10.5:11, color:C.txtMuted }}>
                {previewHtml
                  ? (isMobile ? "Touchez ??? pour imprimer" : "Cliquez ??? dans la topbar pour imprimer ce document")
...
const NAV_PROVISEUR = [...NAV_ADMIN, {id:"departements", emoji:"???", label:"Départements", sub:"Matières · Animateurs"}];
...
const NAV_PROVISEUR_GROUPS = [
  ...
  { section:"ACTEURS & PÉDAGOGIE", items:[
    {id:"enseignants", emoji:"??", label:"Enseignants"},
    {id:"eleves",      emoji:"??", label:"Élèves"},
    {id:"departements",emoji:"???", label:"Départements & Matières", expandable:true,
      sub:[{emoji:"??",label:"SVT"},{emoji:"??",label:"Mathématiques"},{emoji:"??",label:"Sciences Physiques"},{emoji:"??",label:"Lettres"},{emoji:"??",label:"Sciences Humaines"},{emoji:"???",label:"Langues Vivantes"}]},
    {id:"documents",   emoji:"??", label:"Documents"},
  ]},
...
const NAV_CENSEUR_GROUPS = [
  ...
  { section:"DISCIPLINE & VIE SCOLAIRE", items:[
    {id:"sanctions",emoji:"??", label:"Sanctions"},
  ...
  return(
    <div style={{padding:"20px 20px 40px",display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,padding:18}}>
        <div style={{display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10}}>
          <div>
            <h3 style={{margin:"0 0 2px",fontSize:13,fontWeight:700,color:C.txt}}>??? Vue par département</h3>
      ...
      {!loading && (stats.parDept.some(d=>d.nbAlerte>0) || stats.tauxMoyen<50) && (
        <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,padding:18}}>
          <h3 style={{margin:"0 0 12px",fontSize:13,fontWeight:700,color:C.txt}}>?? Alertes & points de vigilance</h3>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
...
function genererBilanTrimestriel(stats, vieSco, sgClasses, niveauLabel, data) {
  ...
  classes.forEach(cl => {
    ...
    eleves.forEach((e,i) => {
      rows += '<tr style="background:'+(i%2===0?"#fff":"#f8fafc")+'">'
        ...
        + '<td style="padding:7px 12px;border-bottom:1px solid #e2e8f0;text-align:center">'+(e.sanctions>0?'?? '+e.sanctions:e.incidents>0?'?? '+e.incidents:'—')+'</td>'
        + '</tr>';
    });
  });
  ...
  const html = '<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8">'
    ...
    + '</head><body>'
    + '<div class="no-print" style="margin-bottom:16px"><button onclick="window.print()" style="padding:10px 20px;background:#0B4D2C;color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:14px;font-weight:700">??? Imprimer / Exporter PDF</button></div>'
  ...
  return(
    <div style={{padding:"20px 20px 40px",display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
        <div>
          <h2 style={{fontSize:20,fontWeight:800,color:C.txt,margin:0}}>Surveillance générale ???</h2>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        ...
        <KpiCard label="Élèves en alerte" value={stats.nbAlerte} sub="3+ absences" iconEmoji="??" bg={C.redPale} subColor={C.red} loading={loading} delay={0.05}/>
        ...
        <KpiCard label="Sanctions" value={vieSco.filter(v=>v.type==="sanction").length} sub="Enregistrées" iconEmoji="??" bg={C.redPale} subColor={C.red} loading={vieLoading} delay={0.15}/>
      </div>
      ...
      {showForm && (
        <div style={{background:C.white,borderRadius:12,border:"1px solid "+C.border,padding:18,display:"flex",flexDirection:"column",gap:12}}>
          <h3 style={{margin:0,fontSize:13,fontWeight:700,color:C.txt}}>
            {tab==="retards"?"? Retard":tab==="sanctions"?"?? Sanction":"?? Incident"}
          </h3>
      ...
      {tab==="absences" && (
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1.4fr 1fr",gap:14}}>
          ...
          <div style={{background:C.white,borderRadius:12,border:"1px solid "+C.border,padding:16}}>
            <h3 style={{margin:"0 0 12px",fontSize:12.5,fontWeight:700,color:C.txt}}>{sgClasses?"?? Par classe":"??? Par département"}</h3>
  ...
  return(
    <div style={{padding:"20px 20px 40px",display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        ...
        <KpiCard label="Enseignants en retard" value={stats.nbAlerteProg} sub="< 50% couverture" iconEmoji="??" bg={C.redPale} subColor={C.red} loading={loading} delay={0.05}/>
  ...
  return(
    <div style={{padding:"20px 20px 40px",display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        ...
        <KpiCard label="Absences cette semaine" value={stats.absWeek} sub="dans le département" iconEmoji="??" bg={stats.absWeek>5?"#fef2f2":C.greenPale} subColor={stats.absWeek>5?C.red:C.green} loading={loading} delay={0.2}/>
      </div>
...
const SidebarSG = ({collapsed, setCollapsed}) => {
  ...
  const NAV = [
    ...
    {id:"retards-sg",  emoji:"??", label:"Billets de retard",     tab:"retards"},
    {id:"sanctions-sg",emoji:"??", label:"Discipline & Sanctions",tab:"sanctions"},
      ...
      <div onClick={()=>setPage("settings")}
        ...
        {!effectiveCollapsed&&(
          <div style={{overflow:"hidden"}}>
            <div style={{fontSize:12,fontWeight:700,color:"#fff",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{user?.nom}</div>
            <div style={{fontSize:10,color:"rgba(255,255,255,.4)"}}>{roleLabel} · ?? Paramètres</div>
          </div>
        )}
      </div>
    ...
    <aside style={{
      ...
      <div onClick={()=>setPage("settings")}
        ...
        {!effectiveCollapsed&&(
          <div style={{overflow:"hidden"}}>
            <div style={{fontSize:12,fontWeight:700,color:"#fff",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{user?.nom}</div>
            <div style={{fontSize:10,color:"rgba(255,255,255,.4)"}}>Proviseur · ?? Paramètres</div>
          </div>
        )}
      </div>
    ...
    <aside style={{
      ...
      <div onClick={()=>setPage("settings")} title="Paramètres — photo de profil, mot de passe"
        ...
        {!effectiveCollapsed && (
          <div style={{overflow:"hidden"}}>
            <div style={{fontSize:12, fontWeight:700, color:"#fff", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"}}>{user?.nom}</div>
            <div style={{fontSize:10, color:"rgba(255,255,255,.4)", textTransform:"capitalize"}}>{user?.role==="proviseur"?"Proviseur":user?.role==="surveillant_general"?"Surveillance générale":user?.role==="censeur"?"Censeur":(user?.role==="animateur"||user?.role==="animatrice")?"Animateur pédagogique":"Enseignant"} · ?? Paramètres</div>
          </div>
        )}
      </div>
    </aside>
  ...
  const rowsHtml = statsEns.map((ens,i) => `
    <tr style="background:${i%2?"#fafafa":"#fff"}">
      ...
      <td style="padding:8px 10px;border:1px solid #ddd;text-align:center">
        ${ens.tauxGlobal>=75?"? Objectif":ens.tauxGlobal>=50?"?? En cours":"?? Alerte"}
      </td>
...
<body>
  ...
  ${enAlerte>0?`<div style="margin-top:16px;padding:10px 14px;background:#fef2f2;border:1px solid #fecaca;border-radius:8px;font-size:11px;color:#991b1b"><strong>?? Enseignants en alerte :</strong> ${statsEns.filter(e=>e.tauxGlobal<50).map(e=>e.nom).join(", ")}</div>`:""}
    ...
    <div style={{padding: isMobile?12:20, display:"flex", flexDirection:"column", gap: isMobile?10:14}}>
      ...
      <div style={{display:"grid", gridTemplateColumns: isMobile?"repeat(2,1fr)":"repeat(4,1fr)", gap:10}}>
        {[
          ...
          {label:"En alerte <50%",  val:enAlerte,    col:enAlerte>0?C.red:C.green, bg:enAlerte>0?C.redPale:C.greenPale, emoji:"??"},
        ].map((k,i)=>(
              ...
              <div key={i} style={{background:C.white, borderRadius:12, border:`1px solid ${C.border}`,
                ...
                <div style={{display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:8, fontSize:11}}>
                  ...
                  <div style={{background:"#e0f2fe",borderRadius:8,padding:"6px 9px", gridColumn:"span 2"}}>
                    <div style={{color:"#0369a1",fontSize:9.5}}>??? Digitalisation</div>
    ...
    <div style={{padding:20, maxWidth:720, margin:"0 auto", display:"flex", flexDirection:"column", gap:16}}>
      ...
      <div style={{background:C.white, borderRadius:12, border:`1px solid ${C.border}`, padding:"16px 18px", display:"flex", flexDirection:"column", gap:12}}>
        ...
        <div style={{background:"#fffbeb", border:"1px solid #fde68a", borderRadius:9, padding:"11px 14px", fontSize:11.5, color:"#92400e"}}>
          ?? <strong>Attention :</strong> l'import remplace TOUTE la liste actuelle de la classe sélectionnée. Vérifiez l'aperçu avant de confirmer. Cette action est définitive.
        </div>
      </div>
...
function DepartementsPage() {
  ...
  return (
    <div style={{padding:"20px 20px 40px", display:"flex", flexDirection:"column", gap:16}}>
      <div>
        <h2 style={{fontSize:18, fontWeight:800, color:C.txt, margin:0}}>??? Départements & Matières</h2>
                    ...
                    {loading ? <Sk h={16} w="60%"/> : deptMatieres.length===0 ? (
                      ...
                      <div key={m.id} style={{display:"flex", alignItems:"center", gap:8, padding:"6px 8px", background:"#f8fafc", borderRadius:7, marginBottom:4}}>
                        ...
                        <button onClick={()=>supprimerMatiere(m)} disabled={savingId===m.id}
                          ...
                          {savingId===m.id ? <Spinner size={11} color={C.red}/> : "???"}
                        </button>
                      </div>
                    ))}
...
function LoginPage({onLogin}){
  ...
  const PROFILES = [
    ...
    {key:"sg",         label:"Surveillance Générale", sub:"Vie scolaire & discipline", desc:"Absences, retards, incidents",    emoji:"???", role:"surveillant_general", needsDept:false},
    ...
    <div style={{
      ...
      <div style={{position:"relative",zIndex:2,display:"flex",flexDirection:"column",alignItems:"center",padding:isMobile?"28px 20px 40px":"40px 24px 60px",textAlign:"center"}}>
        ...
        <div style={{display:"flex",gap:isMobile?16:28,marginBottom:36,animation:"fadeUp .6s ease .45s both",flexWrap:"wrap",justifyContent:"center"}}>
          {[["???","Administration"],["??","Enseignants"],["??","Vie scolaire"]].map(([ico,lbl])=>(
